...
# une fenetre circulaire de 50 m de rayon (soit 11x11) avec une somme = 1
mc1 <- focalWeight(rast_ndvi, d=50, type='circle')
# une fenetre circulaire de 50 m de rayon avec poids 0/1
mc2 <- mc1 * sum(mc1>0)
```
...
### 6.4.4. Exemple : mode, nombre de valeurs uniques

A partir de *rast_clc80_niv2*, calculer dans une fenêtre circulaire de 50 m autour
de chaque pixel :
- le mode (valeur majoritaire)
- la diversité (nombre de valeurs différentes)
...
# raster diversite
rast_divers_50m <- focal(x=rast_clc80_niv2, w=mc2, fun=diversity)
# raster mode
rast_mode_50m <- focal(x=rast_clc80_niv2, w=mc2, fun=getmode)
```
...
```{r tmap_diversity}
# raster diversite
tm_shape(rast_divers_50m) +
  tm_raster(style="cat", palette="OrRd", title="N") +
tm_scale_bar(size=0.4, breaks=c(0,1,2)) +
tm_layout(title="Diversity", legend.bg.color = "white")
```
...
tm_shape(rast_mode_50m) +
  tm_raster(style="cat", palette=pal_clc, legend.show=FALSE) +
tm_scale_bar(size=0.4, breaks=c(0,1,2)) +
tm_layout(title="Mode")
```
