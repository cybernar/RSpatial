...
## 2.7. Créer un 'SpatialPointDataFrame' à partir d'un 'data.frame' muni de coordonnées
...
```{r Créer un SpatialPointsDataFrame}
...
## 2.10. Exemple (1/3) : créer 2 polylignes formées de points aléatoires (ensembles de 7 et 5 points)
```{r SpatialLines p1}
...
```{r}
villes <- c("Madrid", "Buenos-Aires", "Paris", "New-York")
char_lat <- c("40°26'00\"N", "34°36'29\"S", "48° 51' 24\" N", "40° 42' 52\"N")
char_lon <- c("3° 41' W", "58° 22' 13\" W", "2°21'07\"E", "74°0'22\"W")
dd_lat <- as.numeric(char2dms(char_lat, "°", "'", "\""))
dd_lon <- as.numeric(char2dms(char_lon, "°", "'", "\""))
leaflet() %>%
  addTiles() %>%
  addMarkers(lng=dd_lon, lat=dd_lat)
```
...
coordinates(sp_pts_villes) <- ~dd_lon+dd_lat
proj4string(sp_pts_villes) <- CRS("+init=EPSG:4326")

# matrice de distance : toutes les compbinaisons 
m <- spDists(sp_pts_villes, longlat=TRUE)
dimnames(m) <- list (c("MAD","BAI","PAR","NYC"), c("MAD","BAI","PAR","NYC"))
m
# distances consécutives
v <- spDists(sp_pts_villes[c(3,1,4,2),], longlat=TRUE, segments=TRUE)
names(v) <- c("MAD-PAR","PAR-NYC", "NYC-BAI")
v
```
