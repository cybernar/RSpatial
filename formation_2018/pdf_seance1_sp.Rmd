...
## 2.11. Exercice: SpatialPolygons (2 entités, 5 et 7 points)

```{r SpatialPolygons p1, eval=FALSE}
...
## 2.12. Exemple (2/3) : SpatialPolygons 

```{r SpatialPolygons p2, eval=FALSE}
sp_polys <- SpatialPolygons(list(polys_1, polys_2))
plot(sp_polys, axes=T)
str(sp_polys)
```
...
## 2.13. Exemple (3/3) : SpatialPolygonsDataFrame

```{r SpatialPolygons p3, eval=FALSE}
# créons la table attributaire (data.frame, 2 colonnes)
NAME=c("ZONE1", "ZONE2")
...
#URL4 <- "http://data.montpellier3m.fr/sites/default/files/ressources/VilleMTP_MTP_ArbresAlign.zip"
#URL5 <- "http://data.montpellier3m.fr/sites/default/files/ressources/OSM_Metropole_parc_et_jardin.json"
...
#download.file(URL4, "data/VilleMTP_MTP_ArbresAlign.zip")
#download.file(URL5, "data/OSM_Metropole_parc_et_jardin.json")
...
#unzip("data/VilleMTP_MTP_ArbresAlign.zip", exdir="data")
```
...
## 6.3. Comment filtrer des données par attribut ?
...
## 6.4. Comment joindre à un objet Spatial*DataFrame les attributs d'une autre table ?
...
## 6.5. Comment trouver dans quel polygone est situé un point ?
...
## 6.6 Exercice : jointure spatiale + sélection
...
- Vous partirez d'un point P1 dont les coordonnées, en Lambert 93, sont : *X=766350, Y=6313470*.
...
- Créez un objet ```SpatialLinesDataFrame``` nommé __sp_rw__ avec 5 entités Lines, à partir des 5 marches aléatoires.
Le système de coordonnées de __sp_rw__ sera Lambert 93 (EPSG:2154).
Chaque entité aura pour attribut : un identifiant unique et la longueur renvoyée par ```SpatialLinesLengths```.
- Avec leaflet, afficher __sp_rw__ sur un fond de carte OpenStreetMap
...
- Exporter __sp_rw_wgs84__ au format KML avec la fonction ```maptools::kmlLines```
...
leaflet(data=sp_rw_wgs84) %>%
  addTiles() %>%
  addPolylines(color = c("cyan", "deeppink" ,"brown", "darkorange", "chartreuse"))

# export KML
kmlLines(sp_rw_wgs84, kmlfile = "data/test_random_walk.kml", 
         col="deeppink", kmlname = "Formation RSpatial")
```
