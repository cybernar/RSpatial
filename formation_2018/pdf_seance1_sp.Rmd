...
df <- data.frame(name, lon, lat, coul, categ)
```
...
## 2.8. Exemple : créer 2 polylignes (ensembles de 7 et 5 points)

```{r SpatialLines p1}
# cree 2 objets Lines avec ID slot = L1 and L2
x1 <- c(3.871, 3.872, 3.873, 3.874, 3.875, 3.876, 3.877)
y1 <- c(43.61, 43.62, 43.61, 43.62, 43.61, 43.62, 43.61)
x2 <- c(3.911, 3.915, 3.916, 3.912, 3.913)
y2 <- c(43.713, 43.712, 43.716, 43.715, 43.711)
...
```{r SpatialLines p2, include=FALSE}
...
path_data <- "D:/bacasable/formation/data"
setwd(path_data)
```
...
```{r DL DATA, eval=FALSE, include=FALSE}
# changer le répertoire de destination
...
download.file(URL1,"MMM_MMM_Limites.zip")
download.file(URL2,"MMM_MMM_LigneTram.zip")
download.file(URL3,"MMM_MMM_ArretsTram.zip")
download.file(URL4,"MMM_MMM_VeloParc.csv")
unzip("MMM_MMM_Limites.zip")
unzip("MMM_MMM_LigneTram.zip")
unzip("MMM_MMM_ArretsTram.zip")
```
...
tm_shape(shp_communes) +
  tm_polygons() +
tm_shape(sp_pts_df) +
  tm_symbols(size=0.4, col="categ", palette=c(A="blue",B="red"), shape=17) 
```

https://www.r-graph-gallery.com/6-graph-parameters-reminder/
http://www.r-graph-gallery.com/42-colors-names/

## 4.4. étendue, barre d'échelle, flèche du nord dans tmap
