...
df_nom2 <- transmute(df_nom,
                 ID = as.numeric(code_clc_niveau_2),
                 LIB_FR = libelle_fr,
...
tm_shape(rast_clc80_niv2) +
  tm_raster(style="cat", palette = df_clc_categ$PAL)
tm_shape(rast_clc_categ) +
  tm_raster("CLC12_D080_NIV2", style="cat", palette = df_clc_categ$PAL)
```

## Utilisation des masques
Un **masque** est une zone d'intérêt en dehors de laquelle les pixels sont ignorés.
La fonction mask permet de définir un masque qu'on pourra ensuite appliquer à d'autre raster.
